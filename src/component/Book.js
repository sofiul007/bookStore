...
function Book({ book, setEditMode, editMode, editData }) {
  const { name, author, thumbnail: img, price, rating, featured, id } = book;
  ...
  return (
    <div className="book-card">
      <img className="h-[240px] w-[170px] object-cover lws-bookThumbnail" src={img} alt="book" />
      <div className="flex-1 h-full pr-2 pt-2 flex flex-col">
        <div className="flex items-center justify-between">
          <span className={`badge-success lws-Badge ${!featured && "hide"}`}>featured</span>
          <div className="text-gray-500 space-x-2">
            <button className="lws-edit " onClick={handleEditData}>
